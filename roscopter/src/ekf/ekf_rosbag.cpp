...
void ROSbagParser::loadParams()
{
  ...
  get_yaml_node("aruco_topic", param_filename_, aruco_topic_);
  get_yaml_node("landmarks_topic", param_filename_, lms_topic_);
...
void ROSbagParser::parseBag()
{
    ...
    for(rosbag::MessageInstance const m  : (*view_))
    {
        ...
        else if (m.isType<geometry_msgs::PoseStamped>() && m.getTopic().compare(aruco_topic_) == 0)
          ekf_.arucoCallback(m.instantiate<geometry_msgs::PoseStamped>());
        else if (m.isType<feature_tracker::ImageFeatures>() && m.getTopic().compare(lms_topic_) == 0)
          ekf_.landmarksCallback(m.instantiate<feature_tracker::ImageFeatures>());
#ifdef UBLOX
